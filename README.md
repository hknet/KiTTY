...
> ⚠️ **Beta release** (`0.85.1.9-beta`). The full KiTTY feature set on a modern, security-patched PuTTY 0.85 core — including a post-quantum key-exchange warning and a console CLI key generator (`kittygen-cli.exe`). Please still read the known issues below.
...
Current release — **[KiTTY 0.85.1.9-beta](https://github.com/hknet/KiTTY/releases/tag/kitty-0.85.1.9-beta)**:

| Download | Use it when |
|---|---|
| **[Installer — per-user (no admin)](https://github.com/hknet/KiTTY/releases/download/kitty-0.85.1.9-beta/KiTTY-0.85.1.9-beta-x64-peruser.msi)** | **Recommended.** Installs for your user only, **no UAC prompt** (`%LOCALAPPDATA%\Programs\KiTTY`). |
| **[Installer — system-wide](https://github.com/hknet/KiTTY/releases/download/kitty-0.85.1.9-beta/KiTTY-0.85.1.9-beta-x64-system.msi)** | All users, into `Program Files` (requires admin). |
| **[Portable ZIP](https://github.com/hknet/KiTTY/releases/download/kitty-0.85.1.9-beta/kitty-0.85.1.9-beta.zip)** | No install — run from a folder or USB stick. Includes `kitty_portable.exe` and all command-line tools. Executables are **not** UPX-packed (antivirus-friendly). |
| **[Portable ZIP (UPX)](https://github.com/hknet/KiTTY/releases/download/kitty-0.85.1.9-beta/kitty-0.85.1.9-beta-upx.zip)** | Same contents with `kitty.exe`/`kitty_portable.exe` UPX-packed for the smallest download. Some antivirus engines dislike UPX — if in doubt, take the standard ZIP. |
| **[Portable ZIP — 32-bit](https://github.com/hknet/KiTTY/releases/download/kitty-0.85.1.9-beta/kitty-0.85.1.9-beta-32bit.zip)** | The same suite built for 32-bit Windows — for systems a 64-bit binary cannot reach. |
| **[ISO image](https://github.com/hknet/KiTTY/releases/download/kitty-0.85.1.9-beta/kitty-0.85.1.9-beta.iso)** | Both builds (64-bit and 32-bit) as plain files on one ISO — mount it into a virtual machine, no network or install needed. |
...
- **Kitty-Session-Manager** (https://github.com/evgeny-test/Kitty-Session-Manager) was updated for this release by [@evgeny-test](https://github.com/evgeny-test)
