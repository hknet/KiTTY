...
| `[Shortcuts]` | Keyboard shortcuts for KiTTY menu actions, e.g. `duplicate={CONTROL}N`; the Tools menu's file-transfer entries are `winscp` (Start WinSCP, default Shift+F3), `sendfile` (Send File (kscp), Ctrl+F3), `getfile` (Get File (kscp), Ctrl+F4) and `filezilla` (Start FileZilla, Shift+F4; works while the Tools menu offers it). |
