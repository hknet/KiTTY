...
KiTTY can keep a registry-mode backup of its settings, sessions, and host keys as **kittynew.sav** files. In registry mode, KiTTY writes a *fresh, timestamped* `kittynew-YYYYMMDD-HHMMSS.sav` when you open a session from the configuration dialog, when you apply **Change Settings** in a running session, and when you add, edit or delete a named proxy — so the filename always reflects when that copy was written — and keeps the newest `[KiTTY] savbackupcount` of them (default 5). By default they live under `%APPDATA%\KiTTY` next to `kitty.ini`; advanced users can override the base path with `[KiTTY] sav=`. The name is `kittynew.sav`, not `kitty.sav`, so an older KiTTY installed side by side never has its backup overwritten.
...
**What a restore brings back — and what it does not.** The export covers your sessions, host keys, host CAs, named proxies, folders and settings, which is what a restore is normally for. It is not a byte-exact copy of the hive, though: window positions and sizes are held as binary values, which the exporter does not write, and kageant's startup-key list (`StartupKeys`) is exported in a form that comes back with the wrong value type and is then read as empty. Very large individual values may also be shortened. So after restoring a `.sav`, expect to place your windows again and to re-add any kageant startup keys; everything else should return as it was.
