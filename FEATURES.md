...
In a **portable** install (kitty.ini authoritative — see the confirmation section) the list lives in kitty.ini instead of the registry, so it travels with the stick: keys inside the install folder are stored as paths relative to it (surviving a drive-letter change), and a key added from elsewhere prompts you to either copy it into the portable  folder or reference it where it is (machine-local). The autostart entry is machine-local in both modes (an absolute path in HKCU\Run), so it stays pinned to the current machine and path rather than following the stick — handy for a fixed or unattended install, and disabling the option removes it. If a remembered key cannot be found at startup (e.g. the stick is on another machine), kageant shows a single tray notice and skips it rather than dropping it from the list.
