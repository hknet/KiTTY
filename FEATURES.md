...
**Portable directory mode** (`kitty_portable.exe` / `savemode=dir`) is not affected by any of the above — it stores sessions as files and its backups are plain file copies. When settings are applied, KiTTY refreshes `Backups\kitty-portable-latest` and keeps timestamped backups such as `Backups\kitty-portable-YYYYMMDD-HHMMSS` under the portable config directory. By default it keeps 5 timestamped backups; set `[KiTTY] portablebackupcount=0` to disable or another number to change retention. The backup contains `kitty.ini`, `Sessions`, `Sessions_Commands`, `Proxies` (named proxy definitions), `Security` (master-password salt/verifier), `SshHostKeys`, `SshHostCAs`, `Commands`, `Folders`, `Launcher`, `PUTTY.RND`, `KiTTYState` and `Jumplist` — your whole configuration, so a restored backup keeps its sessions, proxies, launcher entries and master-password protection intact. The programs themselves and the `Backups` folder are not copied into it. To restore, close KiTTY/kageant/launcher and copy the backup contents back into the portable config directory.
