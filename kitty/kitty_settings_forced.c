...
void save_open_settings_forced(char *filename, Conf *conf) {
     ...
     * the stored key. CONF_scp_auto_pwd is kept as an inert load-only stub. */
    ...
    write_setting_b_forced(sesskey, "ClipboardRequireFocus", conf_get_bool(conf, CONF_clipboard_require_focus));
